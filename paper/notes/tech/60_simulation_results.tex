...
\begin{itemize}
    \item \textbf{Core model functions} in \texttt{computational/R/functions/model.R}. These define
    ...
    \item \textbf{Grid construction helpers} in \texttt{computational/R/functions/grids.R}. These
    provide convenience functions to create evaluation grids over $v$ and over the parameter pair
    $(\log S,\delta)$.
    \item \textbf{Manifest / reproducibility helpers} in \texttt{computational/R/functions/manifest.R}.
...
\subsection{Interpreting Output \& Results}\label{subsec:sim_interpret}
...
The reported statistics are computed from the derived per-cell summary \texttt{cutoffs\_and\_action\_rates.csv} and from the additional results tables/figures
